...
-- ============================================================
-- Functions replacing key PostgreSQL RPC helpers (RLS-like checks)
-- ============================================================
DELIMITER $$

CREATE OR REPLACE FUNCTION fn_is_superadmin(p_user_id CHAR(36))
RETURNS TINYINT
DETERMINISTIC
READS SQL DATA
BEGIN
  DECLARE v_is_super TINYINT DEFAULT 0;
...
CREATE OR REPLACE FUNCTION fn_user_belongs_to_workshop(p_user_id CHAR(36), p_workshop_id CHAR(36))
RETURNS TINYINT
DETERMINISTIC
READS SQL DATA
BEGIN
  DECLARE v_belongs TINYINT DEFAULT 0;

  IF fn_is_superadmin(p_user_id) = 1 THEN
    RETURN 1;
  END IF;
...
CREATE OR REPLACE FUNCTION fn_is_workshop_admin(p_user_id CHAR(36), p_workshop_id CHAR(36))
RETURNS TINYINT
DETERMINISTIC
READS SQL DATA
BEGIN
...
-- 1) RLS does not exist in MariaDB. Use app-layer filters by workshop_id.
-- 2) Encryption RPCs from pgcrypto are not portable; handle encryption in app/service.
-- 3) Types categories.name and tags.name remain globally unique to match migrations.
-- ============================================================
