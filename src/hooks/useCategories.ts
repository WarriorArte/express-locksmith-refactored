...
export function useTags() {
  ...
  return useQuery({
    queryKey: ["tags", currentWorkshop?.id],
    queryFn: async () => {
      if (!currentWorkshop?.id) return [];

      const data = await phpApiRequest<Tag[]>(`/tags.php?workshop_id=${encodeURIComponent(currentWorkshop.id)}`, {
        method: "GET",
      });
...
export function useCreateTag() {
  ...
  return useMutation({
    mutationFn: async (tag: CreateTagInput) => {
      ...
      const data = await phpApiRequest<Tag>("/tags.php", {
        method: "POST",
        body: JSON.stringify({
          ...tag,
          workshop_id: currentWorkshop.id,
