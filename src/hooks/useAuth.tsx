...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const refreshProfile = useCallback(async () => {
    ...
    try {
      const data = await phpApiRequest<CheckResponse>("/auth.php?action=check", {
        method: "GET",
      });
  ...
  const signIn = useCallback(async (email: string, password: string) => {
    try {
      const data = await phpApiRequest<LoginResponse>("/auth.php?action=login", {
        method: "POST",
        body: JSON.stringify({
  ...
  const signOut = useCallback(async () => {
    ...
    try {
      if (token) {
        await phpApiRequest<null>("/auth.php?action=logout", {
          method: "POST",
          headers: {
