...
export function LlaveEstandarUnLado({ config, cortes, inputSide = 'none', inputMargin: inputMarginProp, children }: Props) {
  ...
  const depthStep = config.depthStep ?? 6;
  const valleyWidth = config.valleyWidth ?? 12;
  const crestWidth = config.crestWidth ?? 12;
  ...
  const guiasY = Array.from({ length: maxDepth }).map((_, i) => baseYTop + i * depthStep);
  ...
  const { d: pathD, svgWidth } = (() => {
    ...
    path += `L ${finalEndX},${baseYTop + config.grosorLlave / 2 + 8} L ${finalEndX - 15},${baseYBottom} L ${shoulderWidth},${baseYBottom} `;
  ...
  return (
    <svg width={svgWidth + pad} height={svgHeight} viewBox={`-${pad} ${viewTop} ${svgWidth + pad} ${svgHeight}`} className="drop-shadow-sm max-w-full" xmlns="http://www.w3.org/2000/svg">
      ...
      <path d={pathD} fill="none" style={{ stroke: 'var(--key-stroke-color, hsl(var(--primary)))', strokeWidth: 'var(--key-stroke-width, 3.5)' }} strokeLinejoin="round" strokeLinecap="round" />
      {children}
    </svg>
  );
}
