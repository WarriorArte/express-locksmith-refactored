...
final class SaleController
{
    ...
    /** GET /api/sales?workshop_id=&page=1&per_page=50 */
    public function index(Request $request): JsonResponse
    {
        ...
        $sales = collect($paginator->items())->map(fn ($row) => (array) $row)->all();
        ...
        return ApiResponse::paginated($paginator, $sales);
    }
    ...
    public function show(Request $request, string $id): JsonResponse
    {
        ...
        if (!$sale) {
            return ApiResponse::error('Venta no encontrada', 404);
        }

        if (!$user->canAccessWorkshop($sale['workshop_id'])) {
    ...
    public function destroy(Request $request, string $id = ''): JsonResponse
    {
        ...
        if (!$user->canAdminWorkshop($sale->workshop_id)) {
    ...
    private function fetchFull(string $id): ?array
    {
        ...
        foreach (['cust_id', 'cust_name', 'cust_phone', 'cust_email'] as $key) {
            unset($sale[$key]);
        }
