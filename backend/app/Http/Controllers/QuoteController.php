...
final class QuoteController
{
    ...
    /** GET /api/quotes?workshop_id=&page=1&per_page=50&status= */
    public function index(Request $request): JsonResponse
    {
        ...
        $query = DB::table('quotes as q')
            ...
            ->select('q.*', 'c.name as cust_name', 'c.phone as cust_phone');
        ...
        $quotes = collect($paginator->items())->map(fn ($row) => (array) $row)->all();
        ...
        return ApiResponse::paginated($paginator, $quotes);
    }
    ...
    public function show(Request $request, string $id): JsonResponse
    {
        ...
        if (!$user->canAccessWorkshop($quote['workshop_id'])) {
    ...
    public function destroy(Request $request, string $id = ''): JsonResponse
    {
        ...
        if (!$user->canAdminWorkshop($quote->workshop_id)) {
