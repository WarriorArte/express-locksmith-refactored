...
final class InstallController
{
    ...
    public function handle(Request $request): Response
    {
        ...
        $installedAt = $this->installedAt($lockPath);

        if ($installedAt !== null && !$allowReinstall) {
            return $this->page('Instalador Laravel', $this->installedHtml($installedAt));
        }
    ...
    private function installedAt(string $lockPath): ?string
    {
        if (is_file($lockPath)) {
            return (string) file_get_contents($lockPath);
        }

        try {
            if (
                Schema::hasTable('app_users')
                && Schema::hasTable('profiles')
                && Schema::hasTable('global_user_roles')
                && DB::table('global_user_roles')->where('role', 'superadmin')->exists()
            ) {
                return 'Detectada por base de datos existente';
            }
        } catch (Throwable) {
            return null;
        }
    ...
    private function seedDemoData(): string
    {
        ...
        $seedPath = database_path('legacy/seed_sample_data.sql');
        if (!is_file($seedPath)) {
    ...
    private function installedHtml(string $installedAt): string
    {
        return '
            <div class="card success">
                ...
                <p>Para re-ejecutar el instalador sin borrar datos abre esta misma URL con <code>?reinstall=1</code>.</p>
    ...
    private function formHtml(array $data, array $errors, bool $reinstall): string
    {
        ...
        return '
            <div class="card">
                ...
                '.$errorHtml.'
                <form method="post" action="'.($reinstall ? '?reinstall=1' : '').'">
